...
async function loadBackupHourSetting() {
  ...
  if (patInput && userData.githubPat) patInput.value = '••••••••••••••••';
}

async function saveGithubPat() {
  const input = document.getElementById('github-pat-input');
  const pat = (input?.value || '').trim();
  if (!pat || pat.startsWith('•')) { showToast('Enter a valid PAT'); return; }
  await saveUserData({ githubPat: pat });
  input.value = '••••••••••••••••';
  showToast('GitHub PAT saved');
}

async function saveBackupHour(value) {
  const hour = parseInt(value);
  await saveUserData({ backupHour: hour });
  const msg = document.getElementById('backup-status-msg');
  if (msg) msg.textContent = 'Updating workflow…';
  try {
    await updateWorkflowCron(hour);
    const hStr = `${String(hour).padStart(2,'0')}:00 UTC`;
    showToast(`Backup scheduled for ${hStr}`);
    if (msg) msg.textContent = `✓ Workflow updated — runs once daily at ${hStr}`;
  } catch(e) {
    console.error('Workflow update failed', e);
    if (msg) msg.textContent = `⚠ Could not update workflow: ${e.message}`;
  }
}

async function updateWorkflowCron(hour) {
  const userData = await getUserData();
  const pat = userData.githubPat;
  if (!pat) throw new Error('No GitHub PAT saved — add it above first');

  const owner = 'godwin-euphoric';
  const repo  = 'Timesheet';
  const path  = '.github/workflows/daily-backup.yml';
  const api   = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;

  const getRes = await fetch(api, {
    headers: { Authorization: `Bearer ${pat}`, Accept: 'application/vnd.github+json' }
  });
  if (!getRes.ok) throw new Error(`GitHub API ${getRes.status}: ${getRes.statusText}`);
  const fileData = await getRes.json();

  const current    = atob(fileData.content.replace(/\n/g, ''));
  const newContent = current.replace(/cron: '0 [0-9*]+ \* \* \*'/, `cron: '0 ${hour} * * *'`);
  if (current === newContent) return; // already correct

  const putRes = await fetch(api, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${pat}`, Accept: 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: `Set backup schedule to ${String(hour).padStart(2,'0')}:00 UTC`,
